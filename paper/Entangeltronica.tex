...
with mobility $\mu$, we adopt the empirical form of
Eq.~(\ref{eq:tau_phi})~\cite{roulleau2008}
\begin{equation}
...
\label{eq:imb}
\end{equation}
Figure~\ref{fig:transfer} shows $\mathcal{I}$ vs. gate voltage (via the 
mapping $V_g\to V_0$ of Sec.~\ref{sec:electrostatics}). The response is 
linear, $\mathcal{I}\approx \mathcal{I}_0 + S_V V_g$, with sensitivity 
$S_V\approx\SI{0.48}{V^{-1}}$ and $R^2=0.99997$ in the shallow-lens regime. 
The maximum simulated imbalance is $0.44$ at $V_g\approx\SI{-0.35}{V}$. The 
linearity follows directly from Eq.~(\ref{eq:phase}): $\phi\propto V_0\propto V_g$.
...
For $\mu=2\times10^6\,\text{cm}^2\text{/Vs}$ ($\tau_\phi\approx\SI{12}{ps}$ 
at $4\,$K), the visibility remains $\expt{C}>0.5$ up to $T\approx\SI{10}{K}$. 
At $T=\SI{77}{K}$ (liquid nitrogen), $\tau_\phi$ drops to $\sim\SI{0.5}{ps}$ 
and the interference is washed out ($\expt{C}<0.1$). The operating boundary 
...
\begin{table}[t]
\centering
\caption{Coherence budget for the EQLI node ($t_{\text{transit}}=0.24\,$ps).}
\label{tab:coherence}
\begin{tabular}{@{}cccc@{}}
\toprule
$T$ (K) & $\tau_\phi$ (ps) & $t_{\text{transit}}/\tau_\phi$ & $\expt{C}$ \\
\midrule
$0.1$  & $>100$ & $<0.003$ & $>0.95$ \\
$4$    & $12$   & $0.02$   & $0.92$  \\
$10$   & $3$    & $0.08$   & $0.55$  \\
...
Setting threshold $\mathcal{I}_{\text{th}}=\mathcal{I}_0$, the output is:
\begin{itemize}
  \item $\mathcal{I}>\mathcal{I}_{\text{th}}$ $\Rightarrow$ logic 1 (constructive 
  upper dominance) when $V_1>V_2$;
  \item $\mathcal{I}<\mathcal{I}_{\text{th}}$ $\Rightarrow$ logic 0 when $V_1<V_2$;
...
by $\Delta C \approx C_0(\delta\phi)^2/2$. To maintain $C>0.5$, the rms 
voltage noise on each gate must satisfy 
$\delta V_{\text{rms}} < \hbar v_g/(e S_V \sigma_x) \approx \SI{0.5}{mV}$, a 
specification well within the reach of cryogenic low-noise electronics.
...
\caption{\textbf{Gate-voltage to effective-potential mapping.} (a) Bare 
electrostatic potential $V_{\text{bare}}$ from the Poisson equation for a 
\SI{20}{nm} gate finger at $V_g=\SI{-0.3}{V}$. (b) Screened effective 
potential $V_{\text{eff}}$ after Thomas--Fermi convolution. (c) Amplitude 
$V_0$ and widths $\sigma_{x,y}$ vs. $V_g$, showing the linear shallow-lens 
regime (shaded) used in the quantum simulations.}
\label{fig:poisson}
\end{figure}
...
$\mathcal{I}$ vs. gate voltage $V_g$ (via the mapping of 
Fig.~\ref{fig:poisson}). Linear response in the shallow-lens regime 
($R^2=0.99997$) with sensitivity $S_V\approx\SI{0.48}{V^{-1}}$.}
\label{fig:transfer}
\end{figure}
