...
(Figure~\ref{fig:readout}; \texttt{scripts/readout_sensitivity.py}). For the
...
r\'elevant distance is therefore source-to-detector ($\approx\SI{85}{nm}$), not
the channel length.
...
edges). Reproducible via \texttt{scripts/readout_sensitivity.py} $\rightarrow$
\texttt{results/readout_sensitivity.json}.}
\label{fig:readout}
\end{figure}
