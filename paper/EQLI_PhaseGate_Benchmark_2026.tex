...
with the slit pitch. With $\sigma_y\approx\SI{15}{nm}$ and a slit separation
of $\SI{24}{nm}$, the screened field at the lower slit is a non-negligible
$V_{\mathrm{eff}}(0,\mp\SI{24}{nm})\approx-4.6\,\text{meV}$: about $31\%$
of the on-lens value $-15\,\text{meV}$ at $V_g=\SI{-0.3}{V}$. The gate
therefore modulates — and phase-shifts — \emph{both} arms, and only a
fraction of the voltage swing is truly differential between the paths. The
numerical simulation already treats both arms self-consistently, so every
