...
interference pattern linearly ($R^2=0.9997$) while preserving the coherent interference figure. 
...
$V_{\text{eff}}(x,y)$, (ii) run an explicit 200-realisation stochastic-dephasing ensemble that sets a conservative operating temperature bound $T_{\max}\approx\SI{11}{K}$ for high-mobility 
($\mu>10^6\,\text{cm}^2\text{/Vs}$) heterostructures, and (iii) propose a 
scalable two-input XOR gate architecture using dual independent phase lenses. 
...
ballistic flying-electron logic in InGaAs 2DEGs; the single-electron-transistor
readout ($T\lesssim\SI{1}{K}$), not the interferometer coherence, is identified
as the dominant thermal bottleneck of the architecture.
\end{abstract}
...
pathway to scalable logic. This work instead benchmarks ballistic
flying-electron logic operable up to $T_{\max}\approx\SI{11}{K}$
(conservative white-noise bound), with projected extension to
$\approx\SI{29}{K}$ under spatially correlated noise, and requiring no
...
\paragraph{Scope and Limitations.} Before proceeding, we state explicitly
what this work is and is not. (i) \emph{No experimental data}: every number
reported here is the output of a numerical simulation, and the paper makes
no claim of measurement. (ii) \emph{Material parameters}: the InGaAs values
($m^*=0.042\,m_0$, $\mu=2\times10^6\,\text{cm}^2\text{/Vs}$, the
$\tau_\phi(T)$ scaling) are plausible literature values, not anchored to a
specific measured sample; a real device will require sample-specific
calibration of every parameter. (iii) \emph{Readout bottleneck}: the
interference signal is read out electrically via a single-electron
transistor (SET), which typically operates at $T<1\,\mathrm{K}$ with charge
noise of order $10^{-5}\,e/\sqrt{\mathrm{Hz}}$; the practical temperature
floor of the system is therefore set by the SET readout, not by the
$\approx\SI{11}{K}$ coherence bound of the interferometer itself. These
three caveats are revisited where they matter
(Secs.~\ref{sec:transfer}, \ref{sec:coherence}, \ref{sec:device}).
...
$\Delta V_g=\SI{-0.3}{V}$ yields $V_0\approx\SI{-15}{meV}$ with emerged 
widths $\sigma_x\approx\SI{13}{nm}$ and $\sigma_y\approx\SI{15}{nm}$: 
Thomas--Fermi screening softens the bare gate field by the factor 
$1+q_{TF}/|q|$ with $q_{TF}=\SI{0.11}{\per\nano\meter}$, so the effective 
lens is wider than the bare gate geometry. The widths are extracted 
self-consistently from the numerical $V_{\text{eff}}$ 
(\texttt{electrostatics.py}, Fig.~\ref{fig:poisson}) and replace the toy 
values of earlier drafts; the Gaussian parametrisation of Eq.~(\ref{eq:veff}) 
remains accurate at the operating point while the phenomenological factor
$k_\phi$ is replaced by a physical control knob.

\paragraph{Amplitude calibration caveat.} The strip model of
Eqs.~(\ref{eq:poisson})--(\ref{eq:thomas-fermi}) underestimates the
capacitive coupling of a real three-dimensional gate: it omits the AlGaAs
barrier (which changes the effective gate--2DEG distance), the image
charges in the gate and the screening layer, and the non-locality of the
screening response. The quoted $50~\mathrm{meV/V}$ is therefore a
\emph{calibration parameter} anchored to the operating point
$V_g=\SI{-0.3}{V}\to V_0=\SI{-15}{meV}$, not a first-principles prediction.
What \emph{does} emerge from the calculation is the \emph{shape} of the
lens ($\sigma_x\approx\SI{13}{nm}$, $\sigma_y\approx\SI{15}{nm}$), which
is fixed by the screening physics and drives the selectivity statements of
Secs.~\ref{sec:geometry}; a change of barrier thickness or doping would
rescale the amplitude while leaving the emerged widths essentially
unchanged.

\paragraph{Phase-selectivity limitation.} The emerged lens is broad compared
with the slit pitch. With $\sigma_y\approx\SI{15}{nm}$ and slit pitch
$\SI{24}{nm}$, the screened field at the centre of the lower slit is a
non-negligible fraction of the on-lens depth: the differential selectivity
ratio is $\approx0.69$, not $1.0$ (Sec.~\ref{sec:geometry}). The gate
therefore modulates — and phase-shifts — \emph{both} arms, and only
$\approx69\%$ of the voltage swing is truly differential between the paths.
The numerical simulation already treats both arms self-consistently, so every
transfer and coherence number reported below is exact for the device
modelled; but true single-slit phase selectivity needs a thinner lens, which
a $\SI{20}{nm}$ gate finger cannot fabricate without either needle/focused
gates or a high-permittivity dielectric. We flag this as a fabrication-level
optimisation, not a flaw of the interferometry scheme; a quantitative
exploration of $\sigma$/pitch trade-offs is given in
Sec.~\ref{sec:robust}.
...
\paragraph{Differential selectivity (cross-arm coupling).} The quantitative
cross-talk estimate: with $\sigma_y\approx\SI{15}{nm}$ and slit pitch
$\SI{24}{nm}$, the screened potential at the centre of the \emph{lower}
slit is $V_{\mathrm{eff}}(0,-\SI{12}{nm})\approx-\SI{4.6}{meV}$, i.e.
$\approx\SI{31}{\percent}$ of the on-lens value
$V_{\mathrm{eff}}(0,+\SI{12}{nm})\approx-\SI{15}{meV}$ at
$V_g=\SI{-0.3}{V}$ (\texttt{scripts/compute\_selectivity.py},
\texttt{results/selectivity.json}). The differential selectivity ratio is
therefore $\approx0.69$, not $1.0$. This reduces the \emph{potential}
phase-gate efficiency (only $\approx69\%$ of the swing is differential) but
does not invalidate the gate: the simulation treats both arms
self-consistently, so the transfer slope of Sec.~\ref{sec:transfer} is
exact for the device as modelled. A thinner lens would require needle gates
or high-$\kappa$ dielectrics (Sec.~\ref{sec:electrostatics}).
...
$\mu=2\times10^6\,\text{cm}^2\text{/Vs}$. In the numerical solver of 
Sec.~\ref{sec:coherence} the amplitude of the delta-correlated potential is 
calibrated so that the simulated mean visibility at $T_0$ reproduces the 
analytical anchor. The dephasing strength $\gamma$ is 
...
$S_V=\mathrm{d}\mathcal{I}/\mathrm{d}V_g\approx\SI{-0.91}{V^{-1}}$ (negative) 
and $R^2=0.9997$. The sign is physically fixed: the lens is attractive 
($V_0\propto V_g<0$), so a more negative $V_g$ deepens $V_0$, shifts 
probability toward the upper detector and raises $\mathcal{I}$ 
($\Delta\mathcal{I}\approx+0.27$ per $-0.3\,$V swing); equivalently 
$\mathcal{I}\approx\mathcal{I}_0+|S_V|\,|V_g|$ with 
$|S_V|\approx\SI{0.91}{V^{-1}}$. The recorded scan extends to 
$V_g=\SI{-0.75}{V}$; the maximum simulated imbalance is $0.76$ at this deepest 
...
\paragraph{Regime statement, not a discovery.} The near-unity
$R^2=0.9997$ is not a special device property. It reflects the fact that the
two-bin detector samples a narrow, near-linear shoulder of a single
interference lobe in the shallow-lens regime ($|V_g|\le\SI{0.3}{V}$); the
scan does not cross a fringe maximum or minimum. $R^2$ is therefore a
\emph{regime statement} — the operating window is linear — not a discovery
about the device.
...
$C=(\max-\min)/(\max+\min)$ is evaluated over the full detector line. In 
the coherent single-particle limit the profile tails vanish, so $C\approx1$; 
thermal dephasing fills the tails, making $C(T)$ the coherence observable of 
Sec.~\ref{sec:coherence}. This confirms that the logic output is 
\emph{phase-displacement interference}, not amplitude modulation.
...
$\approx 0.295$ from $\Delta x=\SI{2}{nm}$ downward. Read as a raw box-sum on 
the grid, it drifts ($0.445\to0.376\to0.336\to0.315$). The apparent 
...
\section{Coherence Budget: Conservative White-Noise Bound}
\label{sec:coherence}
...
ensemble of $N_{\text{ens}}=200$ noise realisations at each of five 
temperatures $T\in\{4,10,20,50,77\}\,$K 
(\texttt{scripts/ensemble\_coherence.py}). The noise amplitude is calibrated 
so that the simulated visibility at $T=\SI{4}{K}$ reproduces the analytical 
value (scale factor $s_\phi=22$; the raw spec value $2.32$ produces no 
visible dephasing and was rejected, see methodological note). The mean 
visibility $\expt{C(T)}$ is computed from the ensemble-averaged detector 
profile; its bootstrap standard error and the analytical line of 
Eq.~(\ref{eq:visibility}) are reported in Fig.~\ref{fig:coherence}.

The numerical ensemble confirms the analytical anchor at $T=\SI{4}{K}$ 
($\expt{C}=0.926$ vs.\ $0.931$) and is strictly decreasing with temperature. 
At intermediate temperatures, however, the delta-correlated white-noise 
model dephases faster than the phenomenological exponential: at 
$T=\SI{10}{K}$ we obtain $\expt{C}=0.549$ (vs.\ $0.878$ analytical), and by 
$T=\SI{20}{K}$ the visibility has collapsed to $0.20$. The crossover 
$C=0.5$ is crossed at $T_{\max}\approx\SI{11}{K}$, which we quote as the 
practical operating bound. At the highest temperatures the white-noise model 
enters its strong-scattering limit: the mean visibility saturates at an 
incoherent floor $\expt{C}\approx0.19$ instead of decaying to zero---a 
non-perturbative artefact of the infinite-bandwidth noise, not a physical 
revival (methodological note). The analytical values of 
Eq.~(\ref{eq:visibility}) are retained as the ideal-device upper envelope. 
The operating boundary of the analytical model is
\begin{equation}
...
$N_{\mathrm{ens}}=200$, $s_\phi=22$). $\langle C \rangle_{\mathrm{ana}}$ is 
Eq.~(\ref{eq:visibility}); $\langle C \rangle_{\mathrm{num}}$ is the simulated 
ensemble mean of Fig.~\ref{fig:coherence}.}
\label{tab:coherence}
\begin{tabular}{ccccc}
\hline
$T~(\mathrm{K})$ & $\tau_{\phi}~(\mathrm{ps})$ & $t_{\mathrm{transit}}/\tau_{\phi}$ & $\langle C \rangle_{\mathrm{ana}}$ & $\langle C \rangle_{\mathrm{num}}$ \\
\hline
4   & 12     & 0.020 & 0.931 & 0.926 \\
10  & 3.04   & 0.079 & 0.878 & 0.549 \\
20  & 1.07   & 0.224 & 0.760 & 0.200 \\
50  & 0.271  & 0.886 & 0.393 & 0.198 \\
77  & 0.142  & 1.69  & 0.175 & 0.188 \\
\hline
\end{tabular}
\end{table}

\subsection*{Note on the Dephasing Parameter Revision and Noise Calibration}
Compared to the initial draft (v1), Table~\ref{tab:coherence} and the
accompanying text have been aligned to the exact dephasing relation of
Eq.~\ref{eq:visibility} ($t_{\mathrm{transit}} = 0.24~\mathrm{ps}$) and, more
importantly, to an explicit numerical ensemble. The raw calibration value
$s_\phi=2.32$ specified in the original task is too weak to produce any
measurable dephasing at any temperature; we therefore calibrated the noise
amplitude empirically so that $\langle C\rangle_{\mathrm{num}}(4~\mathrm{K})$
reproduces the analytical anchor $0.931$, yielding $s_\phi=22$
(\texttt{scripts/\_calibrate\_scale.py}, Fig.~\ref{fig:coherence}). The
high-temperature deviation of the numerical curve from the exponential
($\langle C\rangle_{\mathrm{num}}\to0.19$ rather than $\to0$) is a known
strong-scattering limit of the delta-correlated white-noise model: with an
infinite correlation bandwidth the random potential is not perturbatively
weak at large $T$, and the mean profile saturates at an incoherent floor. We
therefore quote the numerical $T_{\max}\approx11~\mathrm{K}$ as the
conservative operating bound and treat the analytical values as the
ideal-device upper envelope. All verification scripts, JSON results, and
simulation sources are tracked in the public GitHub repository.

\paragraph{Why $s_\phi=22$, not $2.32$.} The empirical calibration factor
$s_\phi=22$ was required because the delta-correlated white-noise model
overestimates dephasing when normalised naively to $\tau_\phi(T)$: the two
interfering paths overlap spatially for most of the transit, and only the
\emph{differential} (between-path) phase difference drives the dephasing.
The raw value $s_\phi=2.32$ from the initial spec produces no visible
dephasing at any temperature, revealing a mismatch between the
phenomenological $\tau_\phi$ and the spatial overlap of the wave packet with
the noise field. The calibration is therefore a \emph{consistency fix}, not
a physical prediction.

Figure~\ref{fig:corr} overlays the visibility curves of the white-noise
model and of spatially correlated noise of length $\xi=\SI{5}{nm}$ and
$\SI{10}{nm}$ at the \emph{same local amplitude}
(\texttt{scripts/noise\_correlation.py}). $T_{\max}$ rises from
$\SI{11.4}{K}$ to $\SI{14.8}{K}$ to $\SI{29.2}{K}$, visualising
immediately that the $\approx\SI{11}{K}$ bound of
Table~\ref{tab:coherence} is the conservative worst case, not a ceiling.

\subsection*{Robustness of the numerical budget (referee due diligence)}
\label{sec:robust}

Three referee-level checks quantify how far the headlines above could move
under reasonable modelling choices. A slim multi-panel figure collects the
outcomes (Fig.~\ref{fig:robust}).

\emph{Noiseless limit.} Setting $s_\phi=0$ must recover the deterministic
solver. In the code this is exact, not approximate: the stochastic routine
with zero amplitude reproduces the split-step evolution bit-for-bit (max.
$|\Delta\psi|=0$, norm conserved to $10^{-12}$), and the recovered
full-line contrast is $C_{\mathrm{det}}=0.99999$. The $C_0=0.95$
prefactor of Eq.~(\ref{eq:visibility}) is a phenomenological single-particle
loss factor; the numerical deterministic limit is set by the full-line
convention and is $\approx1$. The two conventions coincide on the
\emph{relative} suppression $\langle C\rangle/C_0$ used throughout, so the
calibration and all reported ratios are convention-independent.

\emph{Ensemble-size convergence.} The $N_{\text{ens}}=200$ results are
converged in $N$: a nested analysis (the same first $N$ realisations for
every $N$, so no re-run noise) gives $C(4~\mathrm{K})=0.9264$ at $N=200$,
stable to $|\Delta C|\le0.010$ for every $N\ge50$
(\texttt{results/ensemble\_convergence.json}). The steepest point,
$T=10~\mathrm{K}$, is ensemble-volatile by construction ($C=0.549$ with
bootstrap $1\sigma=0.022$ at $N=200$; across the nested ladder it scatters
by up to $0.072$). We report that band rather than fitting it away; the
headline $T_{\max}$ inherits only the well-converged anchor at $4~\mathrm{K}$
together with the steep flank at $10~\mathrm{K}$, so the operating bound is
not an artefact of the ensemble size.

\emph{Noise-amplitude sensitivity.} Re-running $N=200$ at $s_\phi=22\pm5$
(\texttt{scripts/scale\_sensitivity.py}) moves the crossover at
$\mathrm{d}T_{\max}/\mathrm{d}s_\phi\approx-0.73~\mathrm{K}$ per unit of
$s_\phi$: $T_{\max}=15.0~\mathrm{K}$, $11.4~\mathrm{K}$,
$7.8~\mathrm{K}$ at $s_\phi=17$, $22$, $27$. A $\pm5$ calibration error
therefore shifts the quoted bound to
$T_{\max}=11.4\pm3.6~\mathrm{K}$ \emph{if the noise is delta-correlated}.

\emph{Spatially correlated noise.} The delta-correlated model is the
\emph{conservative} white limit. Keeping the local noise amplitude fixed and
only adding a spatial correlation length $\xi$ (variance-preserving Gaussian
smoothing in \texttt{stochastic.py}; \texttt{scripts/noise\_correlation.py})
suppresses the differential dephasing: the two interfering paths $—$ only
$\SI{24}{nm}$ apart at the slits $—$ sample progressively common noise, so
a common-mode component cancels from the phase difference. Consequently
$T_{\max}$ \emph{rises} with $\xi$, to $14.8~\mathrm{K}$
($\xi=\SI{5}{nm}$) and $29.2~\mathrm{K}$ ($\xi=\SI{10}{nm}$) from
$11.4~\mathrm{K}$ at $\xi=0$. A finite-range environment therefore degrades
coherence \emph{less} than white noise, and the $\approx11~\mathrm{K}$ bound
holds as a lower bound for any environment with spatial correlation. The
white-noise numbers are the honest worst case reported in
Table~\ref{tab:coherence}.
...
Readout temperature & $<1\,\mathrm{K}$ (SET) & $<1\,\mathrm{K}$ (SET) or $\sim\SI{100}{mK}$ (QPC current) \\
\bottomrule
\end{tabular}
\end{table}
...
\label{eq:xor}
\end{equation}
with $S_V\approx\SI{-0.91}{V^{-1}}$ as in Sec.~\ref{sec:transfer}; the binary 
...
\paragraph{Readout and fan-out constraints.} The imbalance
$\mathcal{I}$ of Eq.~(\ref{eq:xor}) is a difference of probabilities between
two detector bins \emph{for a single electron} — a statistical observable,
not a directly measurable current. Extracting it requires a single-electron
transistor (SET) integrating charge over times $\gg
t_{\mathrm{transit}}\approx\SI{0.24}{ps}$; SETs typically operate at
$T<1\,\mathrm{K}$ with charge noise of order
$10^{-5}\,e/\sqrt{\mathrm{Hz}}$. The readout — not the coherence of the
interferometer — is therefore the dominant thermal bottleneck of the
architecture (Sec.~\ref{sec:device}). Within this constraint the XOR
establishes the phase-encoding physics as a conceptual proof-of-concept,
not an immediate roadmap for classical logic; a practical gate stack needs
an integrated low-temperature charge detector per output, or a different
readout principle.
...
demonstrates linear phase control ($R^2=0.9997$) with a 
$\SI{50}{meV/V}$ gate coupling, a stable continuous read-out 
($\mathcal{I}\approx0.295$), and second-order spatial convergence at 
$\Delta x=\SI{2}{nm}$. By extending 
...
operates with visibility $>0.5$ up to $T_{\max}\approx\SI{11}{K}$ (numerical 
white-noise ensemble; conservative bound) for $\mu>10^6\,\text{cm}^2\text{/Vs}$, and scales to a reversible XOR gate using 
dual independent lenses.

Three referee-driven robustness checks affirm the numbers: the device bound
\emph{rises} under more realistic noise, so $T_{\max}\approx\SI{11}{K}$ is a
conservative lower bound for correlated environments ($\xi$ up to
$\SI{10}{nm}$ pushes it to $\approx\SI{29}{K}$), it is converged in
$N_{\text{ens}}$, and it is insensitive to the calibration within a
$\pm15\%$ scale change. The one structural caveat is the geometry: the
$\approx\SI{15}{nm}$-wide screened lens is broader than the
$\SI{24}{nm}$ slit pitch, so the gate is not perfectly single-slit-selective
and a thinner lens will require needle gates or high-$k$ dielectrics.
...
architecture requires no superconductors and no quantizing magnetic fields;
its practical temperature floor is set by the single-electron-transistor
readout ($T\lesssim\SI{1}{K}$), not by the interferometer coherence bound of
Sec.~\ref{sec:coherence}. Within that floor it remains a pragmatic,
ultralow-power ballistic-logic testbed.
...
The human author served as the central orchestrator, domain editor, and critical auditor. Holding no formal academic credentials in quantum transport, the human author directed the multi-LLM pipeline using the Pyragogy framework. During human review, a critical mathematical mismatch was detected between Table 1 and the narrative text: the claimed residual visibility at $77~\mathrm{K}$ and the exponential law $\langle C \rangle = C_0 \exp(-t_{\mathrm{transit}}/\tau_\phi)$ disagreed. The human author recalculated the analytical values ($\langle C \rangle = 0.931$, $0.878$, $0.760$, $0.393$, $0.175$ at $T = 4$, $10$, $20$, $50$, $77~\mathrm{K}$) and instructed the agents to run an explicit 200-realisation stochastic ensemble to verify them. The ensemble confirmed the anchor at $4~\mathrm{K}$ but revealed a faster-than-exponential decay at intermediate temperatures ($T_{\max}\approx11~\mathrm{K}$), prompting the noise-amplitude calibration and the honest reporting of the strong-scattering limit documented in Sec.~\ref{sec:coherence}. A further referee-driven pass added three due-diligence checks (noiseless limit, ensemble-size and noise-scale convergence, and spatially correlated noise), all reported in Sec.~\ref{sec:robust}.
...
$V_0\approx\SI{-15}{meV}$, $\sigma_x=\SI{13}{nm}$ and $\sigma_y=\SI{15}{nm}$ 
(emerged widths of the screened solution). 
...
with $S_V\approx\SI{-0.91}{V^{-1}}$ and $R^2=0.9997$; the deeper-lens tail 
...
$V_g$. In the coherent limit the full-line contrast is $C\approx1$ (vanishing 
tails); thermal dephasing fills the tails, making $C(T)$ the coherence 
observable of Sec.~\ref{sec:coherence}. This confirms 
...
as a continuous functional is stable ($\approx 0.295$); read as a raw 
box-sum it drifts ($0.445\to0.315$), proving that the apparent 
...
Solid line: analytical visibility of Eq.~(\ref{eq:visibility}) with $p=1.5$ 
(ideal-device upper envelope); shaded band: 1$\sigma$ bootstrap spread over 
200 noise realisations. The numerical device operates with $C>0.5$ up to 
$T_{\max}\approx\SI{11}{K}$.}
\label{fig:coherence}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{../figures/fig_correlation_overlay.pdf}
\caption{\textbf{Correlated vs.\ white noise (same local amplitude).}
$\expt{C(T)}$ for the delta-correlated model ($\xi=0$, the conservative
limit) and for spatially correlated noise of length $\xi=\SI{5}{nm}$,
$\SI{10}{nm}$ (\texttt{scripts/noise\_correlation.py}). Markers: $T_{\max}$
for each model ($11.4\!\rightarrow\!14.8\!\rightarrow\!29.2~\mathrm{K}$).
Correlated environments dephase the two interfering paths progressively
\textit{in common}, so that component cancels from the relative phase; the
white-noise curve is therefore the worst case and the
$\approx\SI{11}{K}$ bound is a lower bound.}
\label{fig:corr}
\end{figure}
...
\caption{\textbf{Robustness of the coherence budget (referee due diligence).}
(a) Noiseless limit, $C(s_\phi=0)$: the zero-noise stochastic solver
reproduces the deterministic run exactly, giving $C_{\mathrm{det}}=0.99999$
(full-line convention); the phenomenological prefactor $C_0=0.95$ is a
single-particle envelope. (b) Ensemble-size convergence: $C(4~\mathrm{K})$
vs.~$N_{\text{ens}}$ (nested, deterministic seeds) is flat to $\pm0.010$
for $N\ge50$; the $10~\mathrm{K}$ point reports its wider honest band
(bootstrap $1\sigma=0.022$). (c) Noise-scale sensitivity: $T_{\max}$ vs.
$s_\phi$, with $\mathrm{d}T_{\max}/\mathrm{d}s_\phi\approx-0.73~\mathrm{K}$,
so $T_{\max}=11.4\pm3.6~\mathrm{K}$ for $s_\phi=22\pm5$. (d) Spatial
correlation: at fixed local amplitude $T_{\max}$ rises with the correlation
length $\xi$ ($11.4\!\rightarrow\!14.8\!\rightarrow\!29.2~\mathrm{K}$ for
$\xi=0,5,10~\mathrm{nm}$), so the white-noise bound is conservative.}
\label{fig:robust}
\end{figure}
